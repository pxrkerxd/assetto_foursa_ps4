...
// Tell TypeScript what our backend data looks like
interface Tag {
  label: string;
  frequency: number;
...
export function TagCloud() {
  ...
  // Fetch the real tags from the backend
  useEffect(() => {
  ...
  return (
    <div className="glass rounded-xl p-6 h-full flex flex-col">
      ...
      <div className="flex flex-1 flex-wrap items-center justify-center gap-2 py-4">
        {loading ? (
          ...
          filteredTags.map((tag) => (
            <button
              ...
              className={cn(
                ...
                hoveredTag === tag.label && "scale-110 z-10"
              )}
            >
              {tag.label}
              {hoveredTag === tag.label && (
                <span className="absolute -top-6 left-1/2 -translate-x-1/2 rounded bg-foreground px-2 py-0.5 text-xs text-background whitespace-nowrap">
                  {Math.round(tag.frequency / 5)} mentions {/* Divide back the display multiplier */}
                </span>
              )}
            </button>
          ))
      ...
      <div className="mt-auto flex items-center justify-center gap-6 border-t border-border pt-4">
